...
class Spot_comb:
    ...
    def find_streight(self):
        ...
        if straight_comb != None:
            combination = 5            
            for i, c in enumerate(self.r_list):
                if c in straight_comb: flash_list.append(self.s_list[i])
                f = Counter(self.s_list)
                straight_flash = f.most_common(1)
                print (flash_list)
                if straight_flash[0][1] == 5: combination = 9                
...
if __name__ == "__main__":
    ...
    x = Spot_comb(rank_list, suit_list)
